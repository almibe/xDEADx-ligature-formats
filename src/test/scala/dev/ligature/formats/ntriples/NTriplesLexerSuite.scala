...
class NTriplesLexerSuite extends FunSuite {
  ...
  test("handle comments with single token") {
    assertEquals(NTriplesLexer.read("  #this is a comment").toList, List())
    assertEquals(NTriplesLexer.read("#this is a comment   \n# so is this\n     #and this").toList, List(NTriplesToken.EndOfLine(), NTriplesToken.EndOfLine()))
